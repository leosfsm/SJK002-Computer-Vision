...
def showInGrid(imgs, figure_size=None, m=None, n=None, show=None, title="",subtitles=None):
    N = len(imgs)
    ...
    if show is not None:
        for i in range(len(imgs)):
            imgs[i] = imgs[i][min(show[0][1], show[1][1]):max(show[0][1], show[1][1]), min(show[0][0], show[1][0]):max(show[0][0], show[1][0])]
